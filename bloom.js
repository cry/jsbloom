(function(bloom) {
    ...
    /*
     * Includes the LZString library
     * Preconfigured settings:
     *      Number of elements: 100,000
     *      Probability of false positives: 1E-5
     *      Bits in filter: 2396265
     *      Number of hashes: 17
     */

    var BUFFER_LEN      = 2396265,
        HASH_ROUNDS     = 17,
    ...
    var bVector         = new Uint8Array(BUFFER_LEN);
    ...
    bloom.hashes = {
        ...
        getIndices: function(str) {
            ...
            /*
             *
             * Referencing Less Hashing, Same Performance: Building a Better Bloom Filter by Adam Kirsch et al.
    ...
    bloom.addEntries = function(arr) {
        for (var i = arr.length - 1; i >= 0; i--) {

            bloom.addEntry(arr[i]);

        };
    }

    bloom.removeEntries = function(arr) {
        for (var i = arr.length - 1; i >= 0; i--) {

            bloom.removeEntry(arr[i]);

        };
    }

    bloom.removeEntry = function(str) {
        var indices = bloom.hashes.getIndices(str);
    ...
    bloom.import = function(lzData, k) {

        try {
            var raw_data = LZString.decompressFromBase64(lzData);
        } catch (err) {
            throw Error("Data passed to loadPrevious must be LZW and URI Component encoded i.e. LZString");
        }
    ...
    bloom.export = function(callback) {

        if (typeof callback !== "function" || typeof callback === "undefined") {
            throw Error("bloom.export expects a callback to be passed");
        };

        var exportString = new Promise(function(resolve, reject) {
            resolve(LZString.compressToBase64(bVector.toString()));
        });

        exportString.then(function(string) {
            callback(string);
        });
